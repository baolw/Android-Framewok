> 资料来源：http://gityuan.com/2016/06/20/stack_trace/

## 一.概述

Android有一套异常处理机制, 分析Crash时最常见的便是先查看其调用栈stackTrace. 对于调用栈, 是从下往上调用的. 其中经常会遇到”Caused by”, 以及”… 8 more”等信息, 具体是什么含义呢, 本文就是解答这个问题.
...
可以看出, Args.run()的1134行抛出异常RuntimeException; 造成这个异常的原因是该方法的1124行代码出现IndexOutOfBoundsException(数组越界). 这两个stack栈底重复的栈帧个数为8个. 既然是重复了8个, 那么展开就等价于:
